...
const NavItem: React.FC<{ to: string; icon: React.ReactNode; label: string; active?: boolean }> = ({ to, icon, label, active }) => (
  <Link href={to} className={`flex items-center px-3 py-2.5 rounded-lg mb-1 transition-colors ${active ? 'bg-blue-50 text-blue-700 font-medium' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}>
      ...
      <div className="p-6">
        ...
        <div className="mt-6 p-3 bg-slate-50 rounded-lg border border-slate-100">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded font-bold">{organization.plan}</span>
          </div>
        </div>
      </div>
        ...
        {user.role === UserRole.ADMIN && (
          <>
            ...
            <NavItem 
              ...
              active={p === '/templates'} 
              label="Templates" 
              icon={<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 01-2-2V5a2 2 0 012-2h3" /></svg>}
            />
          </>
        )}
      </nav>
